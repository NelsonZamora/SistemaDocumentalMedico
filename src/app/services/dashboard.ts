...
export class DashboardService {
  ...
  async getResumenMedico() {
    ...
    const { count: documentosEmitidos } = await supabase.from('documentos').select('*', { count: 'exact', head: true })
      .eq('creado_por', medicoId);
    const { count: pendientes } = await supabase.from('citas_medicas').select('*', { count: 'exact', head: true })
      .eq('medico_id', medicoId).eq('fecha', hoy).eq('estado', 'programada');

    const horaActual = new Date().toTimeString().substring(0, 5);
    const { data: proximaCita } = await supabase.from('citas_medicas').select('*, pacientes(nombres, apellidos)')
      .eq('medico_id', medicoId).eq('fecha', hoy).eq('estado', 'programada')
  ...
  async getResumenAdmin() {
    ...
    const { count: usuariosActivos } = await supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('activo', true);
    const { count: citasHoy } = await supabase.from('citas_medicas').select('*', { count: 'exact', head: true }).eq('fecha', hoy).eq('estado', 'programada');

    return { totalPacientes: totalPacientes || 0, totalDocumentos: totalDocumentos || 0, usuariosActivos: usuariosActivos || 0, citasHoy: citasHoy || 0 };
  }
  ...
  async getResumenAuxiliar() {
    ...
    const { count: pendientes } = await supabase.from('citas_medicas').select('*', { count: 'exact', head: true }).eq('fecha', hoy).eq('estado', 'programada');
    const { count: medicosActivos } = await supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('activo', true).eq('rol', 'medico');

    const { data: ultimosPacientes } = await supabase.from('citas_medicas').select('fecha, hora_inicio, estado, pacientes(nombres, apellidos)')
      .order('fecha', { ascending: false }).order('hora_inicio', { ascending: false }).limit(5);

    return { totalCitasAux: totalCitasAux || 0, citasHoy: citasHoy ||0, medicosActivos: medicosActivos || 0, pendientes: pendientes || 0, ultimosPacientes: ultimosPacientes || [] };
  }
}